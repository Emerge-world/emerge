...
- **Consequences**: Benchmark runs are now reproducible from profile + snapshot alone, and `meta.json` records `persistence_trace` / `oracle_trace` so load and cleanup choices are auditable after the fact. Curated symbolic benchmarks currently depend on the same precedent JSON structure as frozen snapshots, which keeps tooling simple but preserves the long-term option to introduce a richer symbolic artifact later.
