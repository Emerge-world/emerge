...
- **Consequences**: Benchmark semantics are now mode-dependent and explicit instead of silently inheriting seed-matched local state. Closed-world runs (`frozen` / `symbolic`) are reproducible from profile + snapshot alone, while `live` runs may still reuse local seed-scoped oracle/lineage state when the profile enables it. `meta.json` records `persistence_trace` / `oracle_trace` so the effective load and cleanup choices are auditable after the fact. Curated symbolic benchmarks currently depend on the same precedent JSON structure as frozen snapshots, which keeps tooling simple but preserves the long-term option to introduce a richer symbolic artifact later.
