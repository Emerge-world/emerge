...
Autonomy measures proactive, goal-directed behaviour and the agent's capacity to accumulate knowledge and pursue multi-step plans. The component is decomposed into three sub-scores, each aggregating two underlying signals.

\begin{equation}
C_{\text{autonomy}} = 100 \left(
    0.25 \cdot \underbrace{s_{\text{BI}}}_{\substack{\text{behavioral} \\ \text{initiative}}}
    + 0.375 \cdot \underbrace{s_{\text{KA}}}_{\substack{\text{knowledge} \\ \text{accumulation}}}
    + 0.375 \cdot \underbrace{s_{\text{PE}}}_{\substack{\text{planning} \\ \text{effectiveness}}}
\right)
\label{eq:autonomy}
\end{equation}

\paragraph{Behavioral initiative ($s_{\text{BI}}$).} Average of two proactive behavioral signals:
\[
    s_{\text{BI}} = \frac{r_{\text{proactive}} + r_{\text{contingent}}}{2}
\]
\begin{itemize}[nosep]
    \item \textbf{Proactive rate} ($r_{\text{proactive}}$): Fraction of move actions that are ``proactive''---the agent moves \emph{toward} a nearby resource while its hunger is \emph{below} $60$ (not urgently hungry). A move is proactive if the action is \texttt{move}, hunger $< 60$, and a resource in perception matches the movement direction.
    \item \textbf{Environment-contingent rate} ($r_{\text{contingent}}$): Fraction of innovation attempts occurring when hunger $> 60$. Captures innovation driven by environmental pressure.
\end{itemize}

\paragraph{Knowledge accumulation ($s_{\text{KA}}$).} Average of two memory signals:
\[
    s_{\text{KA}} = \frac{r_{\text{semantic}} + r_{\text{compression}}}{2}
\]
\begin{itemize}[nosep]
    \item \textbf{Semantic growth} ($r_{\text{semantic}}$): Mean final semantic memory count divided by \texttt{MEMORY\_SEMANTIC\_MAX} (default: 30), averaged across all agents. Uses the last \texttt{agent\_state} event per agent. Measures how fully agents populated their long-term semantic memory by run end.
    \item \textbf{Compression yield} ($r_{\text{compression}}$): Mean $\min(1,\, \text{learnings} / \text{episode\_count})$ across all \texttt{memory\_compression\_result} events with $\text{episode\_count} > 0$. Measures information density of each memory compression---how many distinct learnings are extracted per episode compressed.
\end{itemize}

\paragraph{Planning effectiveness ($s_{\text{PE}}$).} Average of two planning signals:
\[
    s_{\text{PE}} = \frac{r_{\text{completion}} + r_{\text{activity}}}{2}
\]
\begin{itemize}[nosep]
    \item \textbf{Plan completion rate} ($r_{\text{completion}}$): $\text{subgoals\_completed} / (\text{subgoals\_completed} + \text{subgoals\_failed})$. Measures how reliably the agent resolves the subgoals it creates; defaults to $0$ when no subgoal events are present.
    \item \textbf{Planning activity} ($r_{\text{activity}}$): $\min(1,\, (\text{subgoals\_completed} + \text{subgoals\_failed}) / \text{action\_total})$. Measures the fraction of agent decisions accompanied by subgoal activity.
\end{itemize}

\paragraph{Edge cases.} If no \texttt{agent\_state} events are logged, $r_{\text{semantic}} = 0$. If no \texttt{memory\_compression\_result} events with $\text{episode\_count} > 0$ are present, $r_{\text{compression}} = 0$. If no subgoal events are logged (e.g., \texttt{ENABLE\_EXPLICIT\_PLANNING = False}), both $r_{\text{completion}}$ and $r_{\text{activity}}$ default to $0$; this is intentional, not a bug.
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Autonomy's $s_{\text{PE}}$ is $0$ when planning is disabled.} Runs with \texttt{ENABLE\_EXPLICIT\_PLANNING = False} produce no \texttt{subgoal\_completed} or \texttt{subgoal\_failed} events. Both $r_{\text{completion}}$ and $r_{\text{activity}}$ default to $0$, and planning effectiveness contributes $0$ to Autonomy. This is correct behaviour; compare only runs with the same planning configuration.
...
\paragraph{Autonomy.} No proactive moves, no environment-contingent innovation, no semantic memory data, no compression events, no subgoal events.
\begin{align*}
    s_{\text{BI}} &= (0 + 0)/2 = 0 \\
    s_{\text{KA}} &= (0 + 0)/2 = 0 \\
    s_{\text{PE}} &= (0 + 0)/2 = 0 \\
    C_{\text{autonomy}} &= 100(0.25 \cdot 0 + 0.375 \cdot 0 + 0.375 \cdot 0) = \mathbf{0.00}
\end{align*}
...
Final memory\_semantic (per agent) & 15 / 30 (all 3 agents) \\
Memory compression events & 2 (each: episode\_count = 5, learnings = 2) \\
Subgoals completed / failed & 6 / 2 \\
...
\begin{align*}
    ...
    s_{\text{BI}} &= (0.625 + 0.60)/2 = 0.6125 \\[4pt]
    r_{\text{semantic}} &= (15/30 + 15/30 + 15/30)/3 = 0.50 \\
    r_{\text{compression}} &= \bigl(\min(1,2/5) + \min(1,2/5)\bigr)/2 = 0.40 \\
    s_{\text{KA}} &= (0.50 + 0.40)/2 = 0.45 \\[4pt]
    r_{\text{completion}} &= 6/(6+2) = 0.75 \\
    r_{\text{activity}} &= \min(1,\, 8/100) = 0.08 \\
    s_{\text{PE}} &= (0.75 + 0.08)/2 = 0.415 \\[4pt]
    C_{\text{autonomy}} &= 100(0.25 \times 0.6125 + 0.375 \times 0.45 + 0.375 \times 0.415) \\
                        &= 100(0.1531 + 0.1688 + 0.1556) = \mathbf{47.75}
\end{align*}
...
\begin{align*}
    ...
    &= 20.19 + 10.30 + 13.09 + 12.81 + 6.21 + 9.98 = \boxed{72.58}
\end{align*}

\begin{interpretbox}
    EBS $\approx 73$ falls in the \textbf{technological ecology} tier. Strong Novelty and near-perfect Stability drive the score. Autonomy benefits from good behavioral initiative ($s_{\text{BI}} = 0.61$) and partial knowledge accumulation ($s_{\text{KA}} = 0.45$), but planning activity remains low ($r_{\text{activity}} = 0.08$) relative to total actions. Longevity benefits from full population survival but is tempered by the asymptotic function ($\text{absolute\_longevity} \approx 0.33$ at 600 agent-ticks vs.\ the $\lambda = 1500$ baseline).
\end{interpretbox}
...
Autonomy ($\times 0.13$)    &  0.00 &  0.00 & 47.75 &  6.21 \\
Longevity ($\times 0.15$)   & 47.40 &  7.11 & 66.50 &  9.98 \\
\midrule
\textbf{EBS}                &       & \textbf{18.29} &  & \textbf{72.58} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tikzpicture}[scale=0.035]
    ...
    \draw[red!70!black, thick]  (70, -100) -- ++(15, 0) node[right, font=\small, black] {Example 2 (EBS $\approx 73$)};
    \fill[red!15, opacity=0.5]  (70, -100) rectangle ++(15, 3);
\end{tikzpicture}
\caption{Radar chart comparing the component profiles of Example~1 (EBS~$\approx 18$, low emergence) and Example~2 (EBS~$\approx 73$, rich emergence). Example~1 collapses toward the origin except for Stability and Longevity; Example~2 fills most of the hexagonal area.}
\label{fig:radar}
\end{figure}
...
\begin{enumerate}
    ...
    \begin{itemize}[nosep]
        ...
        \item No \texttt{agent\_state} events $\Rightarrow$ semantic growth $= 0$
        \item No \texttt{memory\_compression\_result} events with $\text{episode\_count} > 0 \Rightarrow$ compression yield $= 0$
        \item No subgoal events $\Rightarrow$ plan completion rate $= 0$, planning activity $= 0$
...
\begin{enumerate}
    \item \textbf{Planning effectiveness is 0 when planning is disabled.} The $s_{\text{PE}}$ sub-score requires \texttt{subgoal\_completed}/\texttt{subgoal\_failed} events, which are only emitted when \texttt{ENABLE\_EXPLICIT\_PLANNING = True}. In planning-disabled runs, the 37.5\% weight assigned to $s_{\text{PE}}$ within Autonomy is effectively unused, reducing Autonomy's practical ceiling. This is honest measurement, not a defect.
...
\begin{enumerate}
    ...
    \item \textbf{Stratify Autonomy comparisons by planning configuration.} The $s_{\text{PE}}$ sub-score is $0$ when \texttt{ENABLE\_EXPLICIT\_PLANNING = False}. Compare Autonomy scores only across runs with the same planning configuration, or normalise $s_{\text{PE}}$ out of the comparison and note the configuration difference.
...
\addlinespace
\multirow{9}{*}{Autonomy} & \multirow{9}{*}{0.13}
    & \multicolumn{3}{l}{\textit{Sub-score: behavioral\_initiative (weight 0.25)}} \\
    & & \quad proactive\_rate          & 0.125  & proactive moves / total moves \\
    & & \quad env\_contingent\_rate    & 0.125  & hungry attempts / total attempts \\
    & & \multicolumn{3}{l}{\textit{Sub-score: knowledge\_accumulation (weight 0.375)}} \\
    & & \quad semantic\_growth         & 0.1875 & mean(memory\_semantic / MEMORY\_SEMANTIC\_MAX) \\
    & & \quad compression\_yield       & 0.1875 & mean(min(1, learnings / episode\_count)) \\
    & & \multicolumn{3}{l}{\textit{Sub-score: planning\_effectiveness (weight 0.375)}} \\
    & & \quad plan\_completion\_rate   & 0.1875 & subgoals\_completed / planning\_signal \\
    & & \quad planning\_activity       & 0.1875 & min(1, planning\_signal / action\_total) \\
\addlinespace
